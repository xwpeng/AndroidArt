...
void callJavaMethod(JNIEnv *env, jobject thiz) {
    jclass clazz = env->FindClass("com/android/studiocompileso/MainActivity");
    if (clazz == NULL) {
...
jstring Java_com_android_studiocompileso_MainActivity_get(JNIEnv *env, jobject thiz) {
    printf("invoke get in c++\n");
    //jobect thiz表示java对象中的this
    callJavaMethod(env, thiz);
    return env->NewStringUTF("Hello from JNI in libjni-test.so !");
}

void Java_com_android_studiocompileso_MainActivity_set(JNIEnv *env, jobject thiz, jstring string) {
