...
void callJavaMethod(JNIEnv *env, jobject thiz) {
    jclass clazz = env->FindClass("com/android/jnitestapp/MainActivity");
    if (clazz == NULL) {
...
jstring Java_com_android_jnitestapp_MainActivity_get(JNIEnv *env, jobject thiz) {
    printf("invoke get in c++\n");
    callJavaMethod(env, thiz);
    return env->NewStringUTF("Hello from JNI in libjni-test.so !");
}

void Java_com_android_jnitestapp_MainActivity_set(JNIEnv *env, jobject thiz, jstring string) {
